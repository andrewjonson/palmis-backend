...
use App\Services\ApiService\v1\CmisService\References\Formula;

class FormulaController extends Controller
{
    use ConsumeExternalService;

    public function __construct(Formula $apiService)
    {
        ...
        $this->middleware('permission:formula-update|admin', [
            'only' => [
                'update',
                'restore'
            ]
        ]);
        $this->middleware('permission:formula-delete|admin', [
            'only' => [
                'delete',
                'forceDelete'
            ]
