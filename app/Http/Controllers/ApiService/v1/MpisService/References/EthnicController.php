...
use App\Services\ApiService\v1\MpisService\References\Ethnic;

class EthnicController extends Controller
{
    use ConsumeExternalService;

    public function __construct(Ethnic $apiService)
    {
        $this->middleware('permission:ethnic-read|admin', [
            'only' => [
                'index',
                'onlyTrashed',
                'searchEthnic'
        ...
        $this->middleware('permission:ethnic-create|admin', [
            'only' => [
                'store'
            ]
        ]);
        $this->middleware('permission:ethnic-update|admin', [
            'only' => [
                'update',
                'restore'
            ]
        ]);
        $this->middleware('permission:ethnic-delete|admin', [
            'only' => [
                'delete',
                'forceDelete'
            ]
    ...
    {
        return $this->apiService->searchEthnic($request->all());
    }
}