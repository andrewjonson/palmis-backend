...
use App\Services\ApiService\v1\MpisService\References\Barangay;

class BarangayController extends Controller
{
    use ConsumeExternalService;

    public function __construct(Barangay $apiService)
    {
        ...
        $this->middleware('permission:barangay-update|admin', [
            'only' => [
                'update',
                'restore'
            ]
        ]);
        $this->middleware('permission:barangay-delete|admin', [
            'only' => [
                'delete',
                'forceDelete'
            ]
    ...
    {
        return $this->apiService->getBarangayById($id);
    }
