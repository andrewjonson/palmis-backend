...
use App\Services\ApiService\v1\MpisService\References\Built;

class BuiltController extends Controller
{
    use ConsumeExternalService;

    public function __construct(Built $apiService)
    {
        $this->middleware('permission:built-read|admin', [
            'only' => [
                'index',
                'onlyTrashed'
            ]
        ]);
        $this->middleware('permission:built-create|admin', [
            'only' => [
                'store'
            ]
        ]);
        $this->middleware('permission:built-update|admin', [
            'only' => [
                'update',
                'restore'
            ]
        ]);
        $this->middleware('permission:built-delete|admin', [
            'only' => [
                'delete',
                'forceDelete'
            ]
