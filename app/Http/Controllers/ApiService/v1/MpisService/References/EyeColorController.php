...
use App\Services\ApiService\v1\MpisService\References\EyeColor;

class EyeColorController extends Controller
{
    use ConsumeExternalService;

    public function __construct(EyeColor $apiService)
    {
        $this->middleware('permission:eyecolor-read|admin', [
            'only' => [
                'index',
                'onlyTrashed'
            ]
        ]);
        $this->middleware('permission:eyecolor-create|admin', [
            'only' => [
                'store'
            ]
        ]);
        $this->middleware('permission:eyecolor-update|admin', [
            'only' => [
                'update',
                'restore'
            ]
        ]);
        $this->middleware('permission:eyecolor-delete|admin', [
            'only' => [
                'delete',
                'forceDelete'
            ]
