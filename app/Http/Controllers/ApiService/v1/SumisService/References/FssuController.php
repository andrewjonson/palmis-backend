...
use App\Services\ApiService\v1\SumisService\References\Fssu;

class FssuController extends Controller
{
    use ConsumeExternalService;

    public function __construct(Fssu $apiService)
    {
        $this->middleware('permission:fssu-read|admin', [
            'only' => [
                'index',
                'onlyTrashed'
            ]
        ]);
        $this->middleware('permission:fssu-create|admin', [
            'only' => [
                'store'
            ]
        ]);
        $this->middleware('permission:fssu-update|admin', [
            'only' => [
                'update',
                'restore'
            ]
        ]);
        $this->middleware('permission:fssu-delete|admin', [
            'only' => [
                'delete',
                'forceDelete'
            ]
