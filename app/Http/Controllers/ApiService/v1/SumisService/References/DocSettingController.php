...
use App\Services\ApiService\v1\SumisService\References\DocSetting;

class DocSettingController extends Controller
{
    use ConsumeExternalService;

    public function __construct(DocSetting $apiService)
    {
        $this->middleware('permission:docsetting-read|admin', [
            'only' => [
                'index',
                'onlyTrashed'
            ]
        ]);
        $this->middleware('permission:docsetting-create|admin', [
            'only' => [
                'store'
            ]
        ]);
        $this->middleware('permission:docsetting-update|admin', [
            'only' => [
                'update',
                'restore'
            ]
        ]);
        $this->middleware('permission:docsetting-delete|admin', [
            'only' => [
                'delete',
                'forceDelete'
            ]
