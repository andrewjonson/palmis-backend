...
use App\Services\ApiService\v1\SumisService\References\FpaoUnit;

class FpaoUnitController extends Controller
{
    use ConsumeExternalService;

    public function __construct(FpaoUnit $apiService)
    {
        $this->middleware('permission:fpaounit-read|admin', [
            'only' => [
                'index',
                'onlyTrashed'
            ]
        ]);
        $this->middleware('permission:fpaounit-create|admin', [
            'only' => [
                'store'
            ]
        ]);
        $this->middleware('permission:fpaounit-update|admin', [
            'only' => [
                'update',
                'restore'
            ]
        ]);
        $this->middleware('permission:fpaounit-delete|admin', [
            'only' => [
                'delete',
                'forceDelete'
            ]
    ...
    {
        return $this->apiService->filterUnit($id);
    }
}